...
module.exports = function(app) {
  ...
  app.io.use = function(regexp, middleware) {
    if (typeof regexp === 'function') {
      ...
      return app.io.middleware.push(regexp);
    } else {
      middleware.regexp = regexp;
      return app.io.middleware.push(middleware);
    }
  };
};
...
function registerRoute(router, middlewares, route, func) {
  ...
  if (previousFunc) { // if another handler exists, adding new in chain
    ...
    // compile all middlewares in chain
      if (middleware.regexp.test(route)) {
        if (previousFunc) {
          previousFunc = router[route] =
              createRouteFunction(previousFunc, middleware);
        } else {
          previousFunc = router[route] =
              createRouteFunction(middleware, route);
        }
      }
    });
