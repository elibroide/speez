...
com.speez.components.Card = (function(){
	...
	function getThreshold(x, y){
		...
		var middleY = game.height * 0.5 - this.options.height * this.background.scale.y * 0.5;
	...
	Card.prototype.returnCard = function() {
		...
		var timeline = new TimelineLite({ onComplete: function(){ this.inputEnabled = true; } });
	...
	Card.prototype.reject = function(color) {
		...
		this.background.tweenColor(color, 0.1);
	};
	...
	Card.prototype.shake = function() {
		...
		timeline.to(this, this.options.shakeTime, { x: this.options.x + this.options.shake, ease: Elastic.easeOut });
		timeline.to(this, this.options.shakeTime, { x: this.options.x - this.options.shake, ease: Elastic.easeOut }, '-=' + this.options.shakeTime * 3 / 4);
