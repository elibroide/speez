...
com.speez.components.Board = (function(){
	...
	Board.prototype.appear = function(color) {
		...
		// if(this.options.x === 0){
		// 	this.area.options.x -= this.options.width;
		// } else {
		// 	this.area.options.x += this.options.width;
		// }
  //   	this.area.resize();
  ...
  //   		this.area.resize() 
  //   	}.bind(this)});
  //   	timeline.to(this.area.options, this.options.appearTime, { x: this.options.x, ease: Bounce.easeOut })
    	return timeline;
	};
	...
	Board.prototype.setCard = function(card, name) {
		...
		timeline = new TimelineLite({ onComplete: onNameComplete });
